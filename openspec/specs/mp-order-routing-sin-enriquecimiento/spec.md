# Ruteo de notificaciones MercadoPago type=order sin enriquecimiento — Especificación
...
**Archivos afectados (cambios completados v1.4.0):**
- `src/Dotar.Gateway/Providers/IWebhookProvider.cs` — interfaz con `RutearSinEnriquecimiento()` y `ExtraerRoutingKeyDesdeNotificacion()`
- `src/Dotar.Gateway/Providers/MercadoPagoProvider.cs` — implementación de métodos nuevos, refactor DRY con `ParsearRoutingKey()`
- `src/Dotar.Gateway/Workers/WebhookDispatcherWorker.cs` — bifurcación en `ProcesarFlujoProveedorAsync()` por tipo, logs en categoría Worker
- Tests: `MercadoPagoProviderTests.cs`, `WebhookDispatcherWorkerTests.cs` — cobertura TDD de 13 tests provider + 5 tests worker

**Estado de verificación:**
- Resultado: PASS_WITH_WARNINGS (311/311 tests, 0 CRITICAL, 2 WARNING de cobertura de logs async)
- PR #9 mergeado a master en v1.4.0
- Deployed a laboratorio (SSH://lab-oficina)
...
|---------|-------|--------|
| 1.0 | 2026-06-24 | Especificación inicial, implementada y verificada en v1.4.0 |
